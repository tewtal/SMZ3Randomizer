    ...
    class Central : Region {
        ...
        public Central(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, 0, 0x781CC, LocationType.Visible, "Power Bomb (Crateria surface)", Logic switch {
                    _ => new Requirement(items => items.CanUsePowerBombs() && items.Has(SpeedBooster) && items.CanFly())
                }),
                new Location(this, 12, 0x78486, LocationType.Visible, "Missile (Crateria middle)", Logic switch {
                    _ => new Requirement(items => items.CanPassBombPassages())
                }),
                new Location(this, 6, 0x783EE, LocationType.Visible, "Missile (Crateria bottom)", Logic switch {
                    _ => new Requirement(items => items.CanDestroyBombWalls())
                }),
                new Location(this, 11, 0x78478, LocationType.Visible, "Super Missile (Crateria)", Logic switch {
                    _ => new Requirement(items => items.CanUsePowerBombs() && items.HasEnergyReserves(2) && items.Has(SpeedBooster))
                }),
                new Location(this, 7, 0x78404, LocationType.Chozo, "Bombs", Logic switch {
