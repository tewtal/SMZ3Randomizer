    ...
    class East : Region {
        ...
        public East(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, 1, 0x781E8, LocationType.Visible, "Missile (outside Wrecked Ship bottom)", Logic switch {
                    Casual => items => items.Has(SpaceJump) || items.Has(SpeedBooster) || items.Has(Grapple),
                    _ => new Requirement(items => true)
                }),
                new Location(this, 2, 0x781EE, LocationType.Hidden, "Missile (outside Wrecked Ship top)", Logic switch {
                ...
                new Location(this, 3, 0x781F4, LocationType.Visible, "Missile (outside Wrecked Ship middle)", Logic switch {
                    Casual => items => items.Has(Super) && items.CanPassBombPassages() && (items.Has(SpaceJump) || items.Has(SpeedBooster) || items.Has(Grapple)),
                    _ => new Requirement(items => items.Has(Super) && items.CanPassBombPassages())
                }),
                new Location(this, 4, 0x78248, LocationType.Visible, "Missile (Crateria moat)", Logic switch {
                    Casual => items => items.Has(SpaceJump) || items.Has(SpeedBooster) || items.Has(Grapple),
                    _ => new Requirement(items => true)
                }),
            };
