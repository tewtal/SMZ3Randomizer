    ...
    class East : Region {
        ...
        public East(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, "Missile (lava room)", LocationType.Hidden, 0x78AE4, Logic switch {
                    _ => new Requirement(items => items.Has(Morph))
                }),
                new Location(this, "Reserve Tank, Norfair", LocationType.Chozo, 0x78C3E, Logic switch {
                ...
                new Location(this, "Missile (Norfair Reserve Tank)", LocationType.Hidden, 0x78C44, Logic switch {
                    Casual => items => items.Has(Morph) && (
                        items.CanFly() || items.Has(Grapple) && (items.Has(SpeedBooster) || items.CanPassBombPassages()) ||
                        items.Has(HiJump) || items.Has(Ice)),
                    _ => new Requirement(items => items.Has(Morph) && items.Has(Super))
                }),
                new Location(this, "Missile (bubble Norfair green door)", LocationType.Visible, 0x78C52, Logic switch {
                ...
                new Location(this, "Missile (bubble Norfair)", LocationType.Visible, 0x78C66),
                new Location(this, "Missile (Speed Booster)", LocationType.Hidden, 0x78C74, Logic switch {
                    Casual => items => items.CanFly() || items.Has(Morph) && (items.Has(SpeedBooster) || items.CanPassBombPassages()) ||
                        items.Has(HiJump) || items.Has(Ice),
                    _ => new Requirement(items => items.Has(Super))
                }),
                new Location(this, "Speed Booster", LocationType.Chozo, 0x78C82, Logic switch {
                    Casual => items => items.CanFly() || items.Has(Morph) && (items.Has(SpeedBooster) || items.CanPassBombPassages()) ||
                        items.Has(HiJump) || items.Has(Ice),
                    _ => new Requirement(items => items.Has(Super))
                }),
                new Location(this, "Missile (Wave Beam)", LocationType.Visible, 0x78CBC, Logic switch {
                    Casual => items => items.CanFly() || items.Has(Morph) && (items.Has(SpeedBooster) || items.CanPassBombPassages()) ||
                        items.Has(HiJump) || items.Has(Ice),
                    _ => new Requirement(items => true)
                }),
                new Location(this, "Wave Beam", LocationType.Chozo, 0x78CCA, Logic switch {
                    Casual => items => items.Has(Morph) && (
