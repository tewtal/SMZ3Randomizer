    ...
    class Crocomire : Region {
        ...
        public Crocomire(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, "Energy Tank, Crocomire", LocationType.Visible, 0x78BA4, Logic switch {
                    Casual => items => items.HasEnergyReserves(1) || items.Has(SpaceJump) || items.Has(Grapple),
                    _ => new Requirement(items => true)
                }),
                new Location(this, "Missile (above Crocomire)", LocationType.Visible, 0x78BC0, Logic switch {
                ...
                new Location(this, "Power Bomb (Crocomire)", LocationType.Visible, 0x78C04, Logic switch {
                    Casual => items => items.CanFly() || items.Has(HiJump) || items.Has(Grapple),
                    _ => new Requirement(items => true)
                }),
                new Location(this, "Missile (below Crocomire)", LocationType.Visible, 0x78C14, Logic switch {
                    _ => new Requirement(items => items.Has(Morph))
                }),
                new Location(this, "Missile (Grapple Beam)", LocationType.Visible, 0x78C2A, Logic switch {
                ...
                new Location(this, "Grapple Beam", LocationType.Chozo, 0x78C36, Logic switch {
                    Casual => items => items.Has(Morph) && (items.CanFly() || items.Has(SpeedBooster) && items.CanUsePowerBombs()),
