    ...
    class Inner : Region {
        ...
        public Inner(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, 140, "Super Missile (yellow Maridia)", LocationType.Visible, 0x7C4AF, Logic switch {
                ...
                new Location(this, 141, "Missile (yellow Maridia super missile)", LocationType.Visible, 0x7C4B5, Logic switch {
                    Casual => items => items.CanPassBombPassages(),
                    _ => new Requirement(items => items.CanPassBombPassages() &&
                        (items.Has(Gravity) || items.Has(Ice) || items.Has(HiJump) && items.CanSpringBallJump()))
                }),
                new Location(this, 142, "Missile (yellow Maridia false wall)", LocationType.Visible, 0x7C533, Logic switch {
                    Casual => items => items.CanPassBombPassages(),
                    _ => new Requirement(items => items.CanPassBombPassages() &&
                        (items.Has(Gravity) || items.Has(Ice) || items.Has(HiJump) && items.CanSpringBallJump()))
                }),
                new Location(this, 143, "Plasma Beam", LocationType.Chozo, 0x7C559, Logic switch {
                ...
                new Location(this, 144, "Missile (left Maridia sand pit room)", LocationType.Visible, 0x7C5DD, Logic switch {
                    Casual => items => items.CanPassBombPassages(),
                    _ => new Requirement(items => items.Has(HiJump) && (items.Has(SpaceJump) || items.CanSpringBallJump()) || items.Has(Gravity))
                }),
                new Location(this, 145, "Reserve Tank, Maridia", LocationType.Chozo, 0x7C5E3, Logic switch {
                    Casual => items => items.CanPassBombPassages(),
                    _ => new Requirement(items => items.Has(HiJump) && (items.Has(SpaceJump) || items.CanSpringBallJump()) || items.Has(Gravity))
                }),
                new Location(this, 146, "Missile (right Maridia sand pit room)", LocationType.Visible, 0x7C5EB, Logic switch {
                    Casual => new Requirement(items => true),
                    _ => items => items.Has(HiJump) || items.Has(Gravity)
                }),
                new Location(this, 147, "Power Bomb (right Maridia sand pit room)", LocationType.Visible, 0x7C5F1, Logic switch {
                    Casual => new Requirement(items => true),
                    _ => items => items.Has(HiJump) && items.CanSpringBallJump() || items.Has(Gravity)
                }),
                new Location(this, 148, "Missile (pink Maridia)", LocationType.Visible, 0x7C603, Logic switch {
                    Casual => items => items.Has(SpeedBooster),
                    _ => new Requirement(items => items.Has(Gravity))
                }),
                new Location(this, 149, "Super Missile (pink Maridia)", LocationType.Visible, 0x7C609, Logic switch {
                    Casual => items => items.Has(SpeedBooster),
                    _ => new Requirement(items => items.Has(Gravity))
                }),
                new Location(this, 150, "Spring Ball", LocationType.Chozo, 0x7C6E5, Logic switch {
                ...
                new Location(this, 151, "Missile (Draygon)", LocationType.Hidden, 0x7C74D, Logic switch {
                    Casual => items => items.CanDefeatBotwoon(Logic),
                    _ => new Requirement(items => items.CanDefeatBotwoon(Logic) && items.Has(Gravity))
                }),
                new Location(this, 152, "Energy Tank, Botwoon", LocationType.Visible, 0x7C755, Logic switch {
                    _ => new Requirement(items => items.CanDefeatBotwoon(Logic))
                }),
                new Location(this, 154, "Space Jump", LocationType.Chozo, 0x7C7A7, Logic switch {
