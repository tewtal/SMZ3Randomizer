    ...
    class Pink : Region {
        ...
        public Pink(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, 14, "Super Missile (pink Brinstar)", LocationType.Chozo, 0x784E4, Logic switch {
                    _ => new Requirement(items => items.CanPassBombPassages() && items.Has(Super))
                }),
                new Location(this, 21, "Missile (pink Brinstar top)", LocationType.Visible, 0x78608),
                new Location(this, 22, "Missile (pink Brinstar bottom)", LocationType.Visible, 0x7860E),
                new Location(this, 23, "Charge Beam", LocationType.Chozo, 0x78614, Logic switch {
                    _ => new Requirement(items => items.CanPassBombPassages())
                }),
                new Location(this, 24, "Power Bomb (pink Brinstar)", LocationType.Visible, 0x7865C, Logic switch {
                ...
                new Location(this, 25, "Missile (green Brinstar pipe)", LocationType.Visible, 0x78676, Logic switch {
                    _ => new Requirement(items => items.Has(Morph) && (items.Has(PowerBomb) || items.Has(Super)))
                }),
                new Location(this, 33, "Energy Tank, Waterway", LocationType.Visible, 0x787FA, Logic switch {
                ...
                new Location(this, 35, "Energy Tank, Brinstar Gate", LocationType.Visible, 0x78824, Logic switch {
