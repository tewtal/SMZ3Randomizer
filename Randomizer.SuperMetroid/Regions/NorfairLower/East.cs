    ...
    class East : Region {
        ...
        public East(World world, Logic logic) : base(world, logic) {
            Locations = new List<Location> {
                new Location(this, "Missile (Mickey Mouse room)", LocationType.Visible, 0x78F30, Logic switch {
                    Casual => new Requirement(items => true),
                    _ => items => items.Has(Morph) && items.CanDestroyBombWalls()
                }),
                new Location(this, "Missile (lower Norfair above fire flea room)", LocationType.Visible, 0x78FCA),
                new Location(this, "Power Bomb (lower Norfair above fire flea room)", LocationType.Visible, 0x78FD2, Logic switch {
                    Casual => new Requirement(items => true),
                    _ => items => items.CanPassBombPassages()
                }),
                new Location(this, "Power Bomb (Power Bombs of shame)", LocationType.Visible, 0x790C0, Logic switch {
                    _ => new Requirement(items => items.CanUsePowerBombs())
                }),
                new Location(this, "Missile (lower Norfair near Wave Beam)", LocationType.Visible, 0x79100, Logic switch {
                    Casual => new Requirement(items => true),
                    _ => items => items.Has(Morph) && items.CanDestroyBombWalls()
                }),
                new Location(this, "Energy Tank, Ridley", LocationType.Hidden, 0x79108, Logic switch {
                    _ => new Requirement(items => items.CanUsePowerBombs() && items.Has(Super) && items.Has(Charge))
                }),
                new Location(this, "Energy Tank, Firefleas", LocationType.Visible, 0x79184)
            };
        }
